...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<void> login(String email, String password) async {
    ...
    try {
      ...
      if (parentModel.accessToken != null && parentModel.parent != null) {
        await _secureStorage.write(
          key: 'auth_token',
          value: parentModel.accessToken,
        );
        await _secureStorage.write(
          key: 'parent_id',
          value: parentModel.parent!.id,
        );
        await _secureStorage.write(
          key: 'parent_email',
          value: parentModel.parent!.email,
        );

        bool hasChild = false;
        try {
          final children = await _apiServices.getChildren(
            parentModel.accessToken!,
          );
          hasChild = children.isNotEmpty;
          await _secureStorage.write(
            key: 'has_child',
  ...
  Future<void> addChildProfile({required String name, required int age}) async {
